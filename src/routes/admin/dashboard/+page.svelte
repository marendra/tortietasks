<script lang="ts">
	...
	import { deleteTopLevelTask } from '$lib/services/firebase_service';
	...
	async function handleDeleteConfirm() {
		...
		await deleteTopLevelTask(deleteTaskId);
...
<div class="space-y-5">
	<div class="flex items-center justify-between">
		<h2 class="text-lg font-semibold text-text">Tasks</h2>
		<a
			href="/admin/tasks/new"
			class="rounded-lg bg-accent px-3.5 py-2 text-[13px] font-medium text-white transition-colors hover:bg-accent-hover"
		>
			New task
		</a>
	</div>
	...
	<div class="flex flex-wrap gap-2">
		...
		>
			...
			{#each usersStore.users.filter((u) => u.role === 'Employee') as user}
		...
		<div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
			{#each filteredTasks as task (task.id)}
				<TaskCard
					...
					isAdmin={true}
					onClick={() => goto(`/admin/tasks/${task.id}`)}
